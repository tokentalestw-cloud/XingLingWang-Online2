...
# ===== XLW V10 Optional Persistent Room Storage =====
# V10 目標：
# - 若有 REDIS_URL：使用 Redis 保存房間快照與事件紀錄。
# - 若沒有 REDIS_URL：使用 data/runtime_rooms.json 做單機檔案保存。
# - Render 免費版沒有 Redis 時仍可運作；未來接 Redis 不必重寫 WebSocket 架構。

class RoomStorage:
    def __init__(self, base_path: Path):
        self.base_path = base_path
        self.runtime_file = base_path / "data" / "runtime_rooms.json"
    ...
    async def init(self):
        ...
        try:
            ...
            print("[V10] Redis storage enabled.")
        except Exception as e:
            self.redis = None
            self.redis_ready = False
            print(f"[V10] Redis not available; fallback to file storage. reason={e}")
    ...
    def _write_file_all(self, data):
        try:
            ...
            self.runtime_file.write_text(json.dumps(data, ensure_ascii=False), encoding="utf-8")
        except Exception as e:
            print(f"[V10] file storage write failed: {e}")

    async def load_room(self, room_id: str):
        if self.redis_ready and self.redis:
            try:
                raw = await self.redis.get(f"xlw:room:{room_id}")
                return json.loads(raw) if raw else None
            except Exception as e:
                print(f"[V10] redis load failed: {e}")
        data = self._read_file_all()
        return data.get(room_id)

    async def save_room(self, room_id: str, room_data: dict):
        # WebSocket objects 不可序列化，只保存純資料 room_state/event_logs。
        safe = json.loads(json.dumps(room_data, ensure_ascii=False))
        if self.redis_ready and self.redis:
            try:
                await self.redis.set(f"xlw:room:{room_id}", json.dumps(safe, ensure_ascii=False), ex=60 * 60 * 8)
                return
            except Exception as e:
                print(f"[V10] redis save failed: {e}")
        data = self._read_file_all()
        data[room_id] = safe
    ...
    async def delete_room(self, room_id: str):
        if self.redis_ready and self.redis:
            try:
                await self.redis.delete(f"xlw:room:{room_id}")
            except Exception:
                pass
        data = self._read_file_all()
        if room_id in data:
...
room_storage = RoomStorage(BASE)

@app.on_event("startup")
async def xlw_startup_storage():
    await room_storage.init()

class ConnectionManager:
    """
    V9 Reliable Sync Core

    目標：
    1. Server 端保存每個房間最新快照與事件序號。
    2. 每個 Client 操作先經 Server 排序再廣播。
    3. Client 重連時，Server 立即要求對手送最新完整狀態，並盡可能使用既有快照恢復。
    4. 不再讓 sessionStorage 舊資料優先於 Server 狀態。
    5. 仍保持與現有前端事件相容，不一次重寫 43 種 action，降低破壞風險。

    注意：
    - 這是 V9「穩定同步層」，不是完整 Redis 版。
    - Render 請維持 --workers 1 / WEB_CONCURRENCY=1。
    """
    def __init__(self):
        self.rooms = {}
        self.room_state = {}
    ...
    async def load_persisted_room_if_needed(self, room_id: str):
        if room_id in self.room_state and self.room_state[room_id].get("loaded"):
            return
        saved = await room_storage.load_room(room_id)
        if saved:
            self.room_state[room_id] = saved.get("room_state", {"seq": 0, "event_seq": 0, "states": {}, "last_actor": None})
            self.room_state[room_id]["loaded"] = True
            self.event_logs[room_id] = saved.get("event_logs", [])
            print(f"[V10] Loaded persisted room {room_id}.")
        else:
            self._ensure_room_state(room_id)
            self.room_state[room_id]["loaded"] = True
    ...
    async def persist_room(self, room_id: str):
        try:
            if room_id not in self.room_state:
                return
            await room_storage.save_room(room_id, {
                "room_state": self.room_state.get(room_id, {}),
                "event_logs": self.event_logs.get(room_id, [])
            })
        except Exception as e:
            print(f"[V10] persist room failed: {e}")

    def _ensure_room_state(self, room_id: str):
        if room_id not in self.room_state:
        ...
        if room_id not in self.event_logs:
            self.event_logs[room_id] = []

    def _next_event_seq(self, room_id: str) -> int:
        self._ensure_room_state(room_id)
        self.room_state[room_id]["event_seq"] += 1
        return self.room_state[room_id]["event_seq"]

    async def connect(self, room_id: str, player_id: str, websocket: WebSocket):
        await websocket.accept()
        if room_id not in self.rooms:
            self.rooms[room_id] = {}
        await self.load_persisted_room_if_needed(room_id)
        ...
        if "player1" in self.rooms[room_id] and self.rooms[room_id]["player1"][0] == player_id:
            ...
            if "player1" not in self.rooms[room_id]:
                ...
                await websocket.send_json({"type": "error", "message": "房間已滿或您已在連線中！"})
                await websocket.close()
                return

        print(f"[V9] Room {room_id}: {role_assigned} ({player_id}) connected. reconnect={is_reconnect}")
        ...
        await websocket.send_json({
            ...
            "server_sync_v9": True,
            "server_event_seq": self.room_state[room_id].get("event_seq", 0),
            "server_snapshot_seq": self.room_state[room_id].get("seq", 0),
            "message": f"成功連接房間 {room_id}！角色為 {role_assigned}。"
        })

        # 若 server 已有對手視角快照，直接先恢復畫面。
        await self.send_restore_from_opponent_state(room_id, role_assigned, websocket)
        ...
        if "player1" in self.rooms[room_id] and "player2" in self.rooms[room_id]:
            ...
            if is_reconnect:
                ...
                try:
                    await other_ws.send_json({"type": "opponent_rejoined", "message": "對手已重新連線！正在重新同步戰局。"})
                except Exception:
                    pass
                try:
                    await websocket.send_json({"type": "opponent_rejoined_ack", "message": "已成功重新連接到對決，正在同步最新狀態。"})
                except Exception:
                    pass
                await self.broadcast_to_other(room_id, player_id, {
                    "action": "request_full_state",
                    "reason": "opponent_reconnected",
                    "server_request": True
                })
            else:
                try:
                    await p1_ws.send_json({"type": "opponent_joined", "message": "對手已進入房間，遊戲即將開始！"})
                except Exception:
                    pass
                try:
                    await p2_ws.send_json({"type": "opponent_joined", "message": "已連接到房主，遊戲即將開始！"})
                except Exception:
                    pass

    async def send_restore_from_opponent_state(self, room_id: str, role: str, websocket: WebSocket):
        self._ensure_room_state(room_id)
        states = self.room_state[room_id].get("states", {})
        opp_role = "player2" if role == "player1" else "player1"
        opp_msg = states.get(opp_role)
        if not opp_msg:
            return
        try:
            msg = self.flip_message_coordinates(opp_msg)
            msg["server_restore"] = True
            msg["server_seq"] = self.room_state[room_id].get("seq", 0)
            await websocket.send_json(msg)
        except Exception as e:
            print(f"[V9] restore failed: {e}")
    ...
    def disconnect(self, room_id: str, player_id: str):
        if room_id in self.rooms:
            ...
            if "player1" in r and r["player1"][0] == player_id:
                r.pop("player1", None)
                print(f"[V9] Room {room_id}: Player1 disconnected.")
            elif "player2" in r and r["player2"][0] == player_id:
                r.pop("player2", None)
                print(f"[V9] Room {room_id}: Player2 disconnected.")

            if not r.get("player1") and not r.get("player2"):
                self.rooms.pop(room_id, None)
                print(f"[V9] Room {room_id} has no active sockets; state kept for reconnect.")

    def sender_role(self, room_id: str, sender_id: str):
        r = self.rooms.get(room_id, {})
        if "player1" in r and r["player1"][0] == sender_id:
            return "player1"
        if "player2" in r and r["player2"][0] == sender_id:
            return "player2"
        return None

    def save_snapshot_if_needed(self, room_id: str, sender_id: str, message: dict):
        if message.get("action") != "sync_game_state":
            return
        role = self.sender_role(room_id, sender_id)
        if not role:
            return
        self._ensure_room_state(room_id)
        self.room_state[room_id]["seq"] += 1
        self.room_state[room_id]["last_actor"] = role
        snapshot = json.loads(json.dumps(message))
        snapshot["server_seq"] = self.room_state[room_id]["seq"]
        snapshot["server_saved_role"] = role
        self.room_state[room_id]["states"][role] = snapshot
        try:
            asyncio.create_task(self.persist_room(room_id))
        except Exception:
            pass

    def save_event(self, room_id: str, sender_id: str, message: dict) -> dict:
        self._ensure_room_state(room_id)
        role = self.sender_role(room_id, sender_id)
        event = json.loads(json.dumps(message))
        event["server_event_seq"] = self._next_event_seq(room_id)
        event["server_sender_role"] = role
        event["server_time"] = time.time()
        self.event_logs[room_id].append(event)
        if len(self.event_logs[room_id]) > self.max_events:
            self.event_logs[room_id] = self.event_logs[room_id][-self.max_events:]
        try:
            asyncio.create_task(self.persist_room(room_id))
        except Exception:
            pass
    ...
    async def broadcast_to_other(self, room_id: str, sender_id: str, message: dict):
        self.save_snapshot_if_needed(room_id, sender_id, message)
        event = self.save_event(room_id, sender_id, message)
        ...
        if room_id in self.rooms:
            ...
            sender_ws = None
            if "player1" in r and r["player1"][0] == sender_id:
                sender_ws = r["player1"][1]
                if "player2" in r:
                    target_ws = r["player2"][1]
            elif "player2" in r and r["player2"][0] == sender_id:
                sender_ws = r["player2"][1]
                if "player1" in r:
                    target_ws = r["player1"][1]

            # 給送出者 ack，前端可顯示已經由伺服器排序接收
            if sender_ws:
                try:
                    await sender_ws.send_json({
                        "type": "server_ack",
                        "action": message.get("action"),
                        "server_event_seq": event.get("server_event_seq"),
                        "server_snapshot_seq": self.room_state.get(room_id, {}).get("seq", 0)
                    })
                except Exception:
                    pass

            if target_ws:
                flipped_message = self.flip_message_coordinates(event)
                flipped_message["server_seq"] = self.room_state.get(room_id, {}).get("seq", 0)
...
@app.get("/health")
def health_check():
    return {"status": "ok", "service": "xinglingwang-v8-sync"}
    ...
    return {
        "rooms": {
            rid: {
                ...
                "snapshot_seq": manager.room_state.get(rid, {}).get("seq", 0),
                "event_seq": manager.room_state.get(rid, {}).get("event_seq", 0),
                "events": len(manager.event_logs.get(rid, []))
            }
...
@app.post("/api/debug/rooms/{room_id}/clear")
async def clear_debug_room(room_id: str):
    manager.rooms.pop(room_id, None)
    manager.room_state.pop(room_id, None)
    manager.event_logs.pop(room_id, None)
    await room_storage.delete_room(room_id)
    return {"status": "success", "message": f"room {room_id} cleared"}

@app.get("/api/debug/storage")
def debug_storage():
    return {
        "redis_ready": getattr(room_storage, "redis_ready", False),
        "redis_url_set": bool(getattr(room_storage, "redis_url", "")),
        "runtime_file": str(room_storage.runtime_file),
        "runtime_file_exists": room_storage.runtime_file.exists()
    }
...
def delete_deck(data: DeckDeleteSchema):
    ...
    # Do not allow deleting the 5 base faction decks
    base_factions = ["藝術品", "喵喵賊", "妖怪村莊", "獸人", "虛擬世界"]
...
def rename_deck(data: DeckRenameSchema):
    ...
    if not has_faction:
        raise HTTPException(status_code=400, detail="新牌組名稱必須包含所屬種族名稱（例如：喵喵賊、妖怪村莊、藝術品、獸人、虛擬世界）！")
